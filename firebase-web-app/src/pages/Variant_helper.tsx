...
type CaddScore = { raw_score: number; phred: number };

export default function VariantHelper() {
  const [mode, setMode] = useState<'coords' | 'gene'>('coords');
  ...
  const [score, setScore] = useState<CaddScore | null>(null);
  ...
  async function handleAnalyze() {
    ...
    try {
      let url = '/api/cadd-score?';

      if (mode === 'coords') {
        const { chromosome, position, ref, alt } = coords;
        if (!chromosome || !position || !ref || !alt) {
          throw new Error('Please fill chromosome, position, ref, and alt.');
        }
        // Build variant=chr-pos-ref-alt
        const variantStr = `${chromosome}-${position}-${ref}-${alt}`;
        url += `variant=${encodeURIComponent(variantStr)}`;
      } else {
        const { gene, cdna_num, ref, alt } = geneInput;
        if (!gene || !cdna_num || !ref || !alt) {
          throw new Error('Please fill chromosome, position, ref, and alt.');
        }
        const base = /^[ACGT]$/i;
        if (!base.test(ref) || !base.test(alt)) {
          throw new Error('Ref/Alt must be A/C/G/T.');
        }

        // Send them separately. No need to put 'c.' here; Python will add it.
        url += `gene=${encodeURIComponent(gene)}&cdna=${encodeURIComponent(cdna_num)}&ref=${ref.toUpperCase()}&alt=${alt.toUpperCase()}`;
        console.log('FE →', url);
      }
      ...
      // Backend returns CADD array
      const arr = Array.isArray(json) ? json : Array.isArray(json?.cadd) ? json.cadd : null;
      if (!arr || !arr.length) throw new Error('Empty CADD response.');

      const first = arr[0];
      const raw = parseFloat(first?.RawScore ?? first?.raw_score);
      const phred = parseFloat(first?.PHRED ?? first?.phred);
      if (Number.isNaN(raw) || Number.isNaN(phred)) throw new Error('CADD scores missing in response.');

      setScore({ raw_score: raw, phred });
    } catch (e: any) {
      setError(e?.message || 'Failed to analyze variant.');
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        <div className="text-center">
          ...
          <p className="text-xl text-blue-200 max-w-3xl mx-auto">
            CADD scoring via genomic coordinates or gene + cDNA
          </p>
        </div>

        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl shadow-lg">
          <Tabs value={mode} onValueChange={(v) => setMode(v as 'coords' | 'gene')}>
            <TabsList className="w-full bg-white/5 border-b border-white/20 rounded-t-xl rounded-b-none">
              ...
              >
                Coordinates (chr / pos / ref / alt)
              </TabsTrigger>
              <TabsTrigger
                value="gene"
                className="data-[state=active]:bg-blue-600 data-[state=active]:text-white text-blue-200"
              >
                Gene + cDNA (SYMBOL : c.xxx)
              </TabsTrigger>
            </TabsList>

            {/* Coordinates mode */}
            <TabsContent value="coords" className="p-6">
              <div className="space-y-6">
                <h3 className="text-white mb-4">CADD via Genomic Coordinates</h3>
                <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-4">
                  <div>
                    <Label htmlFor="chromosome" className="text-blue-200">Chromosome</Label>
                    <Input
                      id="chromosome"
                      value={coords.chromosome}
                      maxLength={2}
                      onChange={(e) => setCoords({ ...coords, chromosome: e.target.value })}
                      placeholder="16"
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      maxLength={30}
                      onChange={(e) => setCoords({ ...coords, position: e.target.value })}
                      placeholder="11255334"
                      className="bg-white/10 border-white/20 text-white placeholder:text-blue-300"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      value={coords.ref}
                      maxLength={1}
                      onChange={(e) => setCoords({ ...coords, ref: e.target.value })}
                      placeholder="C"
                      className="bg-white/10 border-white/20 text-white placeholder:text-blue-300"
                    />
                  </div>
                  ...
                  <div>
                    <Label htmlFor="geneOptional" className="text-blue-200">Gene (Optional)</Label>
                    <Input
                      id="geneOptional"
                      value={coords.gene}
                      maxLength={30}
                      onChange={(e) => setCoords({ ...coords, gene: e.target.value })}
                      placeholder="SOCS1"
                      className="bg-white/10 border-white/20 text-white placeholder:text-blue-300"
                    />
                  </div>
            ...
            {/* Gene + cDNA mode */}
            <TabsContent value="gene" className="p-6">
              <div className="space-y-6">
                <h3 className="text-white mb-4">CADD via cDNA notation</h3>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div>
                    <Label htmlFor="gene" className="text-blue-200">Gene</Label>
                    <Input
                      id="gene"
                      value={geneInput.gene}
                      maxLength={30}
                      onChange={(e) => setGeneInput({ ...geneInput, gene: e.target.value })}
                      placeholder="SOCS1"
                      className="bg-white/10 border-white/20 text-white placeholder:text-blue-300"
                    />
                  </div>
                  <div>
                    <Label htmlFor="cdna_num" className="text-blue-200">Position relative to Gene</Label>
                    <Input
                      id="cdna_num"
                      value={geneInput.cdna_num}
                      maxLength={5}
                      onChange={(e) => setGeneInput({ ...geneInput, cdna_num: e.target.value })}
                      placeholder="240"
                      className="bg-white/10 border-white/20 text-white placeholder:text-blue-300"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      value={geneInput.ref}
                      maxLength={1}
                      onChange={(e) => setGeneInput({ ...geneInput, ref: e.target.value })}
                      placeholder="C"
                      className="bg-white/10 border-white/20 text-white placeholder:text-blue-300"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      value={geneInput.alt}
                      maxLength={1}
                      onChange={(e) => setGeneInput({ ...geneInput, alt: e.target.value })}
                      placeholder="A"
                      className="bg-white/10 border-white/20 text-white placeholder:text-blue-300"
                    />
                  </div>
          ...
          <div className="mt-4 bg-white/5 border border-white/20 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <span className="text-white ml-2 font-mono">{score.raw_score.toFixed(4)}</span>
              </div>
              <div>
                <span className="text-blue-200">PHRED Score:</span>
                <span className="text-white ml-2 font-mono">{score.phred.toFixed(2)}</span>
              </div>
            </div>
            <div className="mt-3">
              <div className="w-full bg-white/10 rounded-full h-2">
                <div
                  ...
                  style={{ width: `${Math.min((score.phred / 30) * 100, 100)}%` }}
                />
              </div>
